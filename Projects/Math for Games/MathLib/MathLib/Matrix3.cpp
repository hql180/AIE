...
Vector3 Matrix3::operator*(Vector3 & rhs)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		for (int j = 0; j < 3; ++j)
		{
			...
			{
			case 0:
				temp.m_x += m[i][j] * arr[j];
				break;
			case 1:
				temp.m_y += m[i][j] * arr[j];
				break;
			case 2:
				temp.m_z += m[i][j] * arr[j];
				break;
			}
		}
	}
	...
	//temp = Vector3(	m[0][0] * rhs.m_x + m[0][1] * rhs.m_y + m[0][2] * rhs.m_z,
	//				m[1][0] * rhs.m_x + m[1][1] * rhs.m_y + m[1][2] * rhs.m_z,
	//				m[2][0] * rhs.m_x + m[2][1] * rhs.m_y + m[2][2] * rhs.m_z);
